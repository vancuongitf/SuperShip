...
import cao.cuong.supership.supership.data.source.remote.network.ApiService
import cao.cuong.supership.supership.data.source.remote.network.CustomCallAdapterFactory
...
open class GoogleClient private constructor(url: String? = null) {

    internal var token: String? = null
    private var baseUrl: String = if (url == null || url.isEmpty()) BuildConfig.BASE_GOOGLE_API_URL else url

    companion object : SingletonHolder<GoogleClient, String>(::GoogleClient) {
        private const val API_TIMEOUT = 10L // 10 minutes
    }

    val service: GoogleApiService
        get() {
            return createService()
        }

    private fun createService(): GoogleApiService {
